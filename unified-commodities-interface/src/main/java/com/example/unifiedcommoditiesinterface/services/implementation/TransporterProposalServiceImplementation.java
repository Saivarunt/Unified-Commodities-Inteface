...
public class TransporterProposalServiceImplementation implements TransporterProposalService{
    ...
    public TransporterProposalListing proposalListing(String _id, User user) {
        ...
        TransporterProposal transporterProposal = transporterProposalRepository.save(new TransporterProposal(null, transportationalRequest, user, ""));
    ...
    public Page<TransporterProposalListing> fetchMadeProposals(User user, Integer page) {
        Page<TransporterProposal> transporterProposal = transporterProposalRepository.findByTransporter(user, PageRequest.of(page, 10));
    ...
    public Boolean approveTransporterProposal(String _id) {
        ...
        mailSenderService.sendEmail(updatedProposal.getTransporter().getProfile().getEmail(), "Transport Proposal Acceptance", 
        ...
        allProposals.stream().filter(val -> val.getStatus().equals("")).forEach(proposal -> {
            ...
            mailSenderService.sendEmail(proposal.getTransporter().getProfile().getEmail(), "Transport Proposal Rejection", 
        ...
        ProductLifecycle productLifecycle = new ProductLifecycle(null, updatedProposal.getRequest().getProduct(), updatedProposal.getRequest().getProduct().getOwner(), updatedProposal.getRequest().getUser(), updatedProposal.getTransporter(), updatedProposal.getRequest().getQuantity(), "", "");
    ...
    public Boolean rejectTransporterProposal(String _id) {
        ...
        mailSenderService.sendEmail(transporterProposal.getTransporter().getProfile().getEmail(), "Transport Proposal Rejection", 
